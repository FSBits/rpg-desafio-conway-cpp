...
Personagem::Personagem(std::string t_nome, std::string t_classe, int t_forca, int t_reflexos, int t_inteligencia, int t_tecnica, int t_moral, int t_sorte, int t_carisma)
    : Entidade(t_nome, 100 + (t_forca * 5)), 
      ...
      tecnica(t_tecnica), moral(t_moral), sorte(t_sorte), carisma(t_carisma) {
        ...
        Item pistola = {"3 OITAO", "Arma de fogo basica.", ARMA, 5, 1, true};
        Item estimulante = {"estimulante", "Injecao de cura rapida.", CONSUMIVEL, 30, 3, false};
...
void Personagem::listarInv(){
    ...
    for(i = 0; i < inv.size(); i++){
        ...
        if(inv[i].quantidade > 0){
            ...
            Estilo::impressaoEscrita("[" + std::to_string(i) + "] " + std::to_string(inv[i].quantidade) + "x " + Estilo::CIANO + inv[i].nome + Estilo::RESET, 25);

            if(inv[i].id == ARMA){
                Estilo::impressaoEscrita(Estilo::MAGENTA + " (Dano: " + std::to_string(inv[i].valorEfeito) + ")\n" + Estilo::RESET, 25);
            } else if(inv[i].id == CONSUMIVEL)
                Estilo::impressaoEscrita(Estilo::MAGENTA + " (Cura: " + std::to_string(inv[i].valorEfeito) + " HP)\n" + Estilo::RESET, 25);
              else
...
bool Personagem::usarItem(int index){
...
void Personagem::ficha(){
    ...
    Estilo::impressaoEscrita(Estilo::AZUL + "TECNICA:      " + Estilo::RESET + std::to_string(tecnica) + " (Engenharia e armas de fogo)", 10);
